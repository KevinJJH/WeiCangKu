...
public class MessageAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(convertView==null){
            ...
            holder.userimg= (ImageView)convertView.findViewById(R.id.lxw_id_item_message_comment_userimg);
        ...
        User author=mDatas.get(position).getAuthor();
        String userimgpath=null;
        if(author.getAvatar()!=null){
            userimgpath=author.getAvatar();
        }
        if(userimgpath!=null){
           // ImageLoader.getmInstance().loaderImage(Constant.USERIMG+userimgpath,holder.userimg,true);
            Glide.with(mContext).load(userimgpath).diskCacheStrategy(DiskCacheStrategy.ALL).into(holder.userimg);
        }

        holder.username.setText(author.getNick());
