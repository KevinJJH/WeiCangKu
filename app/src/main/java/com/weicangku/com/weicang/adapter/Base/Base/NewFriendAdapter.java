...
import com.weicangku.com.weicang.adapter.Base.Base.Base.BaseRecyclerAdapter;
import com.weicangku.com.weicang.adapter.Base.Base.Base.BaseRecyclerHolder;
...
public class NewFriendAdapter extends BaseRecyclerAdapter<NewFriend> {
    ...
    public void bindView(final BaseRecyclerHolder holder, final NewFriend add, int position) {
        holder.setImageView(add == null ? null : add.getAvatar(), R.drawable.head, R.id.iv_recent_avatar);
...
//            if (BmobUserManager.getInstance().querySingleFriend(add.getUid())){//pengjing
...
//            }
    ...
    private void sendAgreeAddFriendMessage(final NewFriend add,final SaveListener listener){
        ...
        map.put("msg",currentUser.getNick()+"同意添加你为好友");//显示在通知栏上面的内容
